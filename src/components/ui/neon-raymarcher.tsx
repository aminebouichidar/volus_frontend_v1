...
vec3 getHolographicMaterial(vec3 normal, vec3 viewDir, float time) {
  ...
  // Indigo/Blue iridescence for Volus AI theme
  vec3 indigoShades = vec3(
    sin(hue) * 0.2 + 0.4,  // Red (low for indigo)
    sin(hue + 2.0) * 0.2 + 0.3,  // Green (low)
    sin(hue + 4.0) * 0.3 + 0.8   // Blue (high)
  );
  
  return indigoShades * fresnel * 1.2;
}
